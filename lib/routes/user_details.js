...

  {
    ...
    options: {
      ...
      handler: async (request) => {
        ...
        if (VerifyRole === "SuperAdmin" || VerifyRole === "Admin") {
          const payload_email = request.payload["email"];
          const students_info = await kDetailsService.deleteUser(payload_email);
            return { data: students_info };
        }else {
          request.payload['email'] = email 
          const students_info = await kDetailsService.deleteUser(request.payload);
            return { data: students_info };
        }
      }
    } 
